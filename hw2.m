%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SCRIPT
%    hw2
%
% Edited by Daniel Zuerbig
% Febuary 2019
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Setup
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
close all; clc; clear all;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Constants
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
constants.fs=44100;                     % Sampling rate in samples per second
constants.durationScale=.2;             % Duration of notes in a scale
constants.durationChord=4;              % Duration of chords
STDOUT=1;                               % Define the standard output stream
STDERR=2;                               % Define the standard error stream

notes{1}.note='C4';
notes{1}.start=0;
notes{1}.duration=constants.durationChord*constants.fs;
notes{1}.velocity=1;
notes{2}.note='E4';
notes{2}.start=0;
notes{2}.duration=constants.durationChord*constants.fs;
notes{2}.velocity=1;
notes{3}.note='G4';
notes{3}.start=0;
notes{3}.duration=constants.durationChord*constants.fs;
notes{3}.velocity=1;

instrument.temperament='Equal';
instrument.sound='Additive';
%instrument.totalTime=find_length_of_notes(notes);

% for just-tempered chords, use the root note and mode to generate
% frequencies rather than a sequence of note names.
instrument.mode = 'Major';

synthTypes={'Additive','Subtractive','FM','Waveshaper'};

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Questions 1--4 - samples
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for cntSynth=1:length(synthTypes)
    for n=1:length(notes)
        instrument.sound=synthTypes{cntSynth};
        freq = get_root_frequency( notes{n}.note ); 
        % I had to make this change to the create sound function to be
        % compatable with the create chord function. 
        [soundSample]=create_sound(instrument, freq, constants);

        fprintf(STDOUT,'For the %s synthesis type...\n',synthTypes{cntSynth})

        fprintf(STDOUT,'Playing a %s',notes{n}.note);
        play(soundSample,constants.fs);
        fprintf('\n');
    end
end % for cntSynth;


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Question 5  - chords
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for cntSynth=1:length(synthTypes)
    % major chords
    instrument.mode = 'Major';
    instrument.sound=synthTypes{cntSynth};

    instrument.temperament='Just';
    [soundMajorChordJust]=create_chord(instrument,notes{1},constants);
    instrument.temperament='Equal';
    [soundMajorChordEqual]=create_chord(instrument,notes{1},constants);
    
    % minor chords
    instrument.mode = 'Minor';

    instrument.temperament='Just';
    [soundMinorChordJust]=create_chord(instrument,notes{1},constants);
    instrument.temperament='Equal';
    [soundMinorChordEqual]=create_chord(instrument,notes{1},constants);
    
    
    
    fprintf(STDOUT,'For the %s synthesis type...\n',synthTypes{cntSynth})
    
    disp('Playing the Just Tempered Major Chord');
    play(soundMajorChordJust,constants.fs);
    disp('Playing the Equal Tempered Major Chord');
    play(soundMajorChordEqual,constants.fs);
    disp('Playing the Just Tempered Minor Chord');
    play(soundMinorChordJust,constants.fs);
    disp('Playing the Equal Tempered Minor Chord');
    play(soundMinorChordEqual,constants.fs);
    fprintf('\n');
    
end % for cntSynth;


